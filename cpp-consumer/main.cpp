...
//    std::string dbname = std::getenv("POSTGRES_DBNAME");
    ...
    std::string user = "desbordante_user";
...
void processTask(TaskConfig const& task, DBManager const& manager) {
    auto params = task.GetParamsIntersection();
    auto algo = boost::algorithm::to_lower_copy(task.GetAlgo());
    auto primitive_type =  boost::algorithm::to_lower_copy(task.GetType());

    el::Loggers::configureFromGlobal("logging.conf");
    std::cout << "create alg instance\n";
    std::unique_ptr<algos::Primitive> algorithm_instance =
        algos::CreateAlgorithmInstance(primitive_type, algo, params);
    std::cout << "task was created\n";
    try {
        task.UpdateStatus(manager, "IN PROCESS");

        unsigned long long elapsedTime;
        const auto& phaseNames = algorithm_instance->GetPhaseNames();
